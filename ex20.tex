...
the \func{check} macro now and see it calls \func{log\_err}, but \func{check} is
\emph{also} using the \verb|...| and \verb|##__VA_ARGS__| to do the call.  That's
how you can pass full \func{printf} style format strings to \func{check}, which 
go to \func{log\_err}, and then make both work like \func{printf}.
