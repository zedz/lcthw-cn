...
\item \file{bstrlib.h} and \file{bstrlib.c} from \href{http://bstring.sourceforge.net/}. Download the .zip file, extract it, and copy just those two files out.
...
\item[bstrlib.o] Comes from \file{bstlib.c} and header file \file{bstlib.h} which
    you already have.
\item[db.o] From \file{db.c} and header file \file{db.c}, and it
...
\item[devpkg] It's not explicitly mentioned, but instead is the target
    ...
    \file{devpkg.c} which contains the \func{main} method for the whole
    program.
\end{description}
