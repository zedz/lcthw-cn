...
actually realized just how bad most of the code was in this book.  After
reading through it for just an hour I decided that it needs to be taken down
...
using stack allocation to heap allocation with \ident{malloc}.  What happens is,
typically \ident{malloc} returns memory from the heap, and so the bytes after
it are not initialized.  Then you see me use a loop to accidentally initialize
it wrong.  Before the switch to heap memory, this program probably ran just fine
...
and not doing the clever triple equality trick.  It also uses an \ident{extern}
