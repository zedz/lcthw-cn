...
\begin{description}
\item[24-28] Set up some variables we'll need.
\item[30-32] Get your first name using the \func{fgets} function, which reads a
    string from the input (in this case \ident{stdin}) but makes sure it
    doesn't overflow the given buffer.
\item[34-36] Same thing for \ident{you.last\_name}, again using \func{fgets}.
\item[38-39] Uses \func{fscanf} to read an integer from \func{stdin} and put it
...
\item[41-45] Print out all the options available for eye color, with a matching
    number that works with the \ident{EyeColor} enum above.
\item[47-50] Using \func{fscanf} again, get a number for the \ident{you.eyes}, 
    ...
    segfault.
\item[52-53] Get how much you make as a \ident{float} for the \ident{you.income}.
\item[55-61] Print everything out so you can see if you have it right.  Notice
...
but fails for strings because it's difficult to tell scanf how big a buffer
