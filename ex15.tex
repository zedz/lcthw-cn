...
\begin{description}
\item[6-10] Create two arrays, \ident{ages} storing some \ident{int}
    data, and \ident{names} storing an array of strings.
\item[12-13] Some variables for our \ident{for-loops} later.
\item[16-19] You know this is just looping through the two arrays and
    printing how old each person is.  This is using \ident{i} to 
    index into the array.
\item[22] Create a pointer that points at \ident{ages}.
...
\item[23] Create a pointer that points at \ident{names}.
    ...
    it to yourself.
\item[26-29] Loop through \ident{ages} and \ident{names} but instead
...
goal is to get your to attempt figuring pointers out for yourself before
...
to think of \ident{ages} is it's the "address of all the first integer in
...
is called a "pointer".  The \ident{cur\_age} and \ident{cur\_name} variables
are pointing at the location where \ident{ages} and \ident{names} live in your
computer's memory.  The example program is then moving them around or doing
math on them to get values out of the memory.
...
\begin{enumerate}
\item Ask the OS for a chunk of memory using \func{malloc} and use a pointer
    to work with it.  This includes strings and something you haven't seen
    yet, \ident{structs}.
\item Passing large block of memory (like strings and arrays) to functions
...
\item[type *ptr = \&thing] "a pointer of type names ptr set to the address of thing"
\item[*(ptr++)] "the value of wherever ptr points, then increment it"
\end{description}
